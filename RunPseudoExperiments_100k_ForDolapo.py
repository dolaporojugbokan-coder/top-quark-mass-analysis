...
OUTPUT_DIR = f"{HOME}/output_closure_ForDolapo"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Fix random seed for reproducibility
np.random.seed(42)

# Load per-bin slopes and intercepts saved by RunParametrisation_ForDolapo.py
slopes, intercepts = [], []
...
for m_true, filename in zip(mass_list, file_list):
    ...
    print(f"\nRunning {N_EXPERIMENTS} pseudo-experiments for m_true = {m_true} GeV (ForDolapo, {nbins} bins)...")

    f_tmp     = ROOT.TFile.Open(f"{FOLDER}/{filename}")
    if not f_tmp or f_tmp.IsZombie():
        raise RuntimeError(f"Could not open {FOLDER}/{filename}")
    h_tmp     = f_tmp.Get("h_mtop_param")
    if not h_tmp:
        f_tmp.Close()
        raise RuntimeError(f"Could not find h_mtop_param in {FOLDER}/{filename}")
    ...
    plt.xlabel("Pull")
    plt.ylabel("Density")
    plt.title(f"Pull distribution for $m_{{top}}$ = {m_true} GeV (ForDolapo, 100k experiments)")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title(f"Fitted $m_{{top}}$ distribution for {m_true} GeV (ForDolapo, 100k experiments)")
    plt.legend()
    plt.tight_layout()
    ...
    plt.ylabel("Experiments")
    plt.title(f"Residual $m_{{top}}^{{fit}} - m_{{top}}^{{true}}$ for {m_true} GeV (ForDolapo, 100k)")
    plt.legend()
    plt.tight_layout()
    ...
    plt.ylabel("Experiments")
    plt.title(f"Upper error distribution for {m_true} GeV (ForDolapo, 100k experiments)")
    plt.legend()
    plt.tight_layout()
    ...
    plt.ylabel("Experiments")
    plt.title(f"Lower error distribution for {m_true} GeV (ForDolapo, 100k experiments)")
    plt.legend()
    plt.tight_layout()
